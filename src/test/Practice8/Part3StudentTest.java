...
public class Part3StudentTest {
    ...
    public void insertUserAndGetAllTest() {
        ...
        assertEquals("[team1, team2]", userTeams.toString());
    }

    @Test
    public void getUserTest() {
        User user = User.createUser("Misha");
        dbManager.insertUser(user);
        User user1 = dbManager.getUser("Misha");
        assertEquals("Pasha", user1.getLogin());
    }
    ...
    public static void afterTest() throws SQLException {
        try(Connection connection = DriverManager.getConnection(DB_URL, USER, PASS);
            Statement statement = connection.createStatement()) {
            String sql1 = "DROP IF EXISTS TABLE users";
            String sql2 = "DROP IF EXISTS TABLE teams";
            String sql3 = "DROP IF EXISTS TABLE users_teams";
